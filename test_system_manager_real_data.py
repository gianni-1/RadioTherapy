#!/usr/bin/env python3
"""
Test SystemManager with real training data
"""
...
def test_system_manager_with_real_data():
    """Test SystemManager with actual training data"""
    print("Testing SystemManager with real training data...")
    
    # Use the same input data as corrected_inference.py
    ...
    if not os.path.exists(input_path):
        ...
        if os.path.exists("traindata/11_5/inputcube/"):
            files = os.listdir("traindata/11_5/inputcube/")[:5]
            for f in files:
                print(f"  {f}")
        return
    ...
    try:
        output_path = system_manager.run_inference(
            input_path,  # Use the real input file
        ...
        if data.shape == ground_truth.shape:
            ...
            print(f"Shape mismatch: output {data.shape} vs ground truth {ground_truth.shape}")
...
if __name__ == "__main__":
    test_system_manager_with_real_data()
