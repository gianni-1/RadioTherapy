...
class MedNISTDataset(Randomizable, CacheDataset):
    ...
    def _generate_data_list(self, dataset_dir: PathLike) -> list[dict]:
        ...
        class_names = sorted(
            f"{x.name}" for x in dataset_dir.iterdir() if x.is_dir()
        )  # folder name as the class name
        self.num_class = len(class_names)
        image_files = [
            [f"{x}" for x in (dataset_dir / class_names[i]).iterdir()]
...
class DecathlonDataset(Randomizable, CacheDataset):
    ...
    def __init__(
        ...
        self._properties = load_decathlon_properties(
            dataset_dir / "dataset.json", property_keys
        )
        if transform == ():
...
class TciaDataset(Randomizable, CacheDataset):
    ...
    def __init__(
        ...
        if download:
            seg_series_list = get_tcia_metadata(
                query=f"getSeries?Collection={collection}&Modality={seg_type}",
            ...
            if len(seg_series_list) == 0:
                raise ValueError(
                    f"Cannot find data with collection: {collection} seg_type: {seg_type}"
                )
        ...
        if transform == ():
            transform = LoadImaged(
                keys=["image"], reader="PydicomReader", fname_regex=self.fname_regex
            )
        CacheDataset.__init__(
            self,
            data=self.datalist,
    ...
    def _download_series_reference_data(
        self, series_uid: str, download_dir: str
    ) -> None:
        """
        ...
        dicom_files = [
            f for f in sorted(os.listdir(seg_first_dir)) if f.endswith(".dcm")
        ]
        # achieve series number and patient id from the first dicom file
        dcm_path = os.path.join(seg_first_dir, dicom_files[0])
        ds = PydicomReader(stop_before_pixels=True, specific_tags=self.load_tags).read(
            dcm_path
        )
        ...
        if not patient_id:
            warnings.warn(
                f"unable to find patient name of dicom file: {dcm_path}, use 'patient' instead."
            )
            patient_id = "patient"
        ...
        if not series_num:
            warnings.warn(
                f"unable to find series number of dicom file: {dcm_path}, use '0' instead."
            )
            series_num = 0

        series_num = str(series_num)
        seg_dir = os.path.join(
            download_dir, patient_id, series_num, self.seg_type.lower()
        )
        ...
        for dcm_file in dicom_files:
            ...
            ds = PydicomReader(
                stop_before_pixels=True, specific_tags=self.load_tags
            ).read(dcm_path)
            if ds[self.modality_tag].value == self.seg_type:
                ...
                if ref_uid == "":
                    ...
                    ref_uid = match_tcia_ref_uid_in_study(
                        ds.StudyInstanceUID, ref_sop_uid
                    )
                if ref_uid != "":
                    ref_uid_list.append(ref_uid)
        if not ref_uid_list:
            warnings.warn(
                f"Cannot find the referenced Series Instance UID from series: {series_uid}."
            )
        else:
            download_tcia_series_instance(
                series_uid=ref_uid_list[0],
                download_dir=download_dir,
                output_dir=dcm_dir,
                check_md5=False,
            )
    ...
    def _generate_data_list(self, dataset_dir: PathLike) -> list[dict]:
        ...
        datalist = []
        patient_list = [
            f.name for f in os.scandir(dataset_dir) if f.is_dir() and f.name != "raw"
        ]
        for patient_id in patient_list:
            series_list = [
                f.name
                for f in os.scandir(os.path.join(dataset_dir, patient_id))
                if f.is_dir()
            ]
...
class CustomDataset(CacheDataset):
    """
    Custom dataset class to load patient data with input and target cubes in .npy format.
    It integrates caching and lazy loading for efficient data handling.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            section (str): Data section, can be 'training', 'validation', or 'test'.
            transform: List of transformations to apply to the data.
            cache_num (int): Number of items to cache.
            cache_rate (float): Percentage of data to cache.
            num_workers (int): Number of worker threads for caching.
            progress (bool): Show progress bar for caching.
            copy_cache (bool): Deepcopy cache content before transforms.
            as_contiguous (bool): Make cached arrays contiguous.
            runtime_cache (bool): Cache data at runtime.
        """
        self.data_dir = Path(data_dir)
        if not self.data_dir.is_dir():
            raise ValueError(f"Data directory {data_dir} is invalid.")
        self.section = section

        # Prepare data list
        data_list = self._prepare_data_list()

        # Set default transformations if not provided
        if transform is None:
            transform = Compose(
                [
                    LoadImaged(keys=["input", "target"]),
                ]
            )
    ...
    def _prepare_data_list(self) -> list:
        ...
        for patient_dir in self.data_dir.iterdir():
            ...
            for cube in cubes_info["cubes"]:
                ...
                energy = cube.get("energy", None)  # Request energy information TODO: Prüfe auf Richtigkeit der Anfrage
                cubeSize = cube.get("cubeSize", None)  # Request cube size information
                resolution = cube.get("resolution", None)  # Request resolution information
...
class CrossValidation:
    ...
    def __init__(
        self, dataset_cls: object, nfolds: int = 5, seed: int = 0, **dataset_params: Any
    ) -> None:
        ...
        class _NsplitsDataset(self.dataset_cls):  # type: ignore
            ...
            def _split_datalist(self, datalist: list[dict]) -> list[dict]:
                data = partition_dataset(
                    data=datalist, num_partitions=nfolds, shuffle=True, seed=seed
                )
